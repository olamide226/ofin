...
func refIndexStore(t *testing.T) *Store {
	...
	if _, err := db.Exec(`CREATE TABLE chunks (
		...
		cross_refs TEXT, source TEXT)`); err != nil {
		t.Fatal(err)
	}
	rows := []struct {
		id       int64
		...
		// whole Minimum Wage Act, prose name.
		{2, "Nigeria Tax Act 2025", "s.58", `["Minimum Wage Act"]`},
		// 3: same-act sectioned self-reference.
		{3, "Nigeria Tax Act 2025", "s.57", `["section 58 of this Act"]`},
	...
	for _, r := range rows {
		if _, err := db.Exec(
			`INSERT INTO chunks (id, act_short, section_id, cross_refs, source) VALUES (?,?,?,?,?)`,
			r.id, r.act, r.sec, r.refs, sources[r.act]); err != nil {
			t.Fatal(err)
		}
	}
	return &Store{db: db}
